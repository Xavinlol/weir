...
/// Absence of the `via` header on a 429 indicates a Cloudflare-level rate limit.
#[inline]
pub fn has_via_header(headers: &axum::http::HeaderMap) -> bool {
...
mod tests {
    ...
    #[test]
    fn via_header_present() {
        let mut headers = HeaderMap::new();
        headers.insert("via", "1.1 google".parse().unwrap());
        assert!(has_via_header(&headers));
    }

    #[test]
    fn via_header_absent() {
